...
class SpaceChicken extends Phaser.Scene {
    ...
    init(data = {}) {
        ...
        if (
            SpaceChicken.backgroundLayouts instanceof Map &&
            SpaceChicken.backgroundLayouts.size > 12
        ) {
    ...
    create() {
        ...
        // Create kill zone
        const killZoneY =
            this.levelConfig.killZoneY +
            (this.levelConfig.killZoneHeight || GAME_CONSTANTS.LEVEL_KILLZONE_HEIGHT);
    ...
    handleInput() {
        ...
        const movementMidpoint =
            this.uiManager && this.uiManager.touchMovementMidpoint
        ...
        activePointers.forEach((pointer) => {
            ...
            const isOnJumpButton =
                hasCoordinates && this.isPointerOverGameObject(pointerX, pointerY, jumpButton);
            const isOnMusicToggle =
                hasCoordinates &&
                this.isPointerOverGameObject(pointerX, pointerY, musicToggleButton);
            const isOnLeaderboardButton =
                hasCoordinates &&
                this.isPointerOverGameObject(pointerX, pointerY, leaderboardButton);
            const pointerEligibleForDoubleTap =
                !isOnJumpButton && !isOnMusicToggle && !isOnLeaderboardButton;
            ...
            if (pointer.justUp) {
                const upTime =
                    typeof pointer.upTime === 'number' && pointer.upTime > 0
                        ...
                        : performance.now();
                if (
                    this.uiManager.touchControlsEnabled &&
                    hasCoordinates &&
                    pointerEligibleForDoubleTap
                ) {
                    const lastTapTime = this.pointerTapTimes.has(pointer.id)
                        ? this.pointerTapTimes.get(pointer.id)
                        : 0;
                    if (
                        lastTapTime > 0 &&
        ...
        const pointerJumpTriggered =
            !jumpButton &&
            activePointers.some((pointer) => {
                if (this.jumpPointerId !== null && pointer.id === this.jumpPointerId) {
                    return false;
                }
                if (!pointer.justUp) {
                ...
                const hasCoordinates = typeof pointerX === 'number' && typeof pointerY === 'number';
                if (hasCoordinates) {
                    const isOnMusicToggle = this.isPointerOverGameObject(
                        pointerX,
                        pointerY,
                        musicToggleButton
                    );
                    const isOnLeaderboardButton = this.isPointerOverGameObject(
                        pointerX,
                        pointerY,
                        leaderboardButton
                    );
